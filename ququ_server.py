...
RULES_TEXT = """【总则】只用权威韦特塔罗牌义解读;数字牌看数字、花色看领域、正逆看顺阻、宫廷看人物、大阿卡纳看命运主题。牌面给什么说什么,不编造不臆造;客观量化用层级/直读表+标准锚点(央行/统计局/国标)。档位:数字牌Ace=1、2-10=牌面数字;宫廷牌侍从=2/骑士=4/王后=6/国王=8;逆位=档位减1(最低1);大阿卡纳不落档、只按牌义定性。
...
【多牌主牌】多牌题只以"主牌"报具体数字,其余牌只给方向(正位往上带、逆位往下带),不做数值加减。主牌判定:问结果/未来→结果位;问现状→现状位;问对方想法→对方想法位;问多久/何时→时机/时间位;问总资产/身家/财富多少→"总资产量级"位或"财富顶点"位;无对应位→最后一张。牌的数量不影响结论:数字只看主牌这一张。
...
【财富·层级】(权威韦特牌义定层级+真实量级;只看主牌"总资产量级/结果/财富顶点"位,其余牌给方向,牌数不影响结论)层级7级(低→高),每级给真实量级数字(央行/胡润锚定,让人有概念):①负债/资不抵债=净资产≤0;②匮乏/手头紧=总资产<50万;③持平/一般=总资产50万~300万(城镇家庭总资产中位163万在此档);④小康/自给自足=总资产300万~600万;⑤富裕/殷实=总资产600万~1000万(胡润高净值门槛);⑥巨富=总资产1000万~1亿;⑦命运级/首富=总资产1亿~100亿,极极端(世界/星币10+明确首富/上市公司/马斯克语境)可到千亿~万亿(马斯克约6.2万亿)。判层级按主牌的权威牌义(不是数字递增):①高塔(indigence/ruin)、死神(loss)、恶魔(物质束缚);②星币5(destitution最硬的穷牌)、宝剑5、宝剑9、宝剑王后(privation);③星币2/3/4/7/8等中性数字牌、权杖/圣杯/宝剑中段、侍从、骑士、月亮(账目不清但非归零);④星币6、星币9(abundance)、太阳(物质幸福)、皇后(丰饶);⑤星币Ace(gold)、星币10(Gain/riches家族财富)、皇帝(世俗权)、命运之轮(机运)、国王(经营才能);⑥世界(确定成功)、星币王后(Opulence);⑦世界/星币10+明确首富语境。大阿卡纳不落档、按状态定性;宫廷牌按成熟度(侍从=新手/骑士=上升/王后=成熟/国王=掌控),但星币王后=富裕、宝剑王后=匮乏。逆位是另一组牌义、不机械降档,按该牌逆位牌义重新定性(如星币10逆=loss/robbery降为②、命运之轮逆=abundance仍可富)。只报"层级名+量级区间",不编造精确到个位/万位的数。
...
【解读要求】
1. 先判断问题类型：金钱题——收入类【收入·牌面直读】,身家/家底/净资产/总资产统一【财富·层级】(专业牌义定层级+真实量级);时间题按花色单位;是/否按正逆牌义；纯感情题不套量表，主观解读。
