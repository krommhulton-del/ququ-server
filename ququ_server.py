...
· 花色修正（仅金钱/身体题，只对主牌生效）：主牌为星币 +1 档、主牌为宝剑 -1 档
...
【财富量化 v6】每档=一个具体锚点数值，解读统一输出"约X元(浮动±10~15%)"，不写宽区间、不说"几万/几千"这种模糊话；按【多牌聚合】定档(先定主牌、其余牌仅修正封顶±2)；大阿卡纳作主牌=不落档(只定性不给具体数)；花色修正(仅金钱题，只对主牌生效)：主牌为星币+1档、主牌为宝剑-1档。
